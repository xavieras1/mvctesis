...
jQuery.jgrid.extend({
	setColumns : function(p) {
		...
		return this.each(function(){
			...
			if(p.recreateForm===true && jQuery("#"+IDs.themodal).html() != null) {
				jQuery("#"+IDs.themodal).remove();
			}
			if ( jQuery("#"+IDs.themodal).html() != null ) {
				if(onBeforeShow) { p.beforeShowForm(jQuery("#"+dtbl)); }
				jQuery.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, jqM:false, modal:p.modal});
				if(onAfterShow) { p.afterShowForm(jQuery("#"+dtbl)); }
			} else {
				...
				p.gbox = "#gbox_"+gID;
				jQuery.jgrid.createModal(IDs,formdata,p,"#gview_"+$t.p.id,jQuery("#gview_"+$t.p.id)[0]);
				$(".clickDespliegue a").click(function() {
 					console.log($(this));
 					//$("#content").load("miembrosinstancia.php?nombre="+$(this).attr("href").substring(11));
 				});
				if(p.saveicon[0]==true) {
					jQuery("#dData","#"+dtbl+"_2").addClass(p.saveicon[1] == "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
					.append("<span class='ui-icon "+p.saveicon[2]+"'></span>");
				}
				if(p.closeicon[0]==true) {
					jQuery("#eData","#"+dtbl+"_2").addClass(p.closeicon[1] == "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
					.append("<span class='ui-icon "+p.closeicon[2]+"'></span>");
				}
				if(!p.updateAfterCheck) {
					jQuery("#dData","#"+dtbl+"_2").click(function(e){
						for(i=0;i<$t.p.colModel.length;i++){
							if(!$t.p.colModel[i].hidedlg) { // added from T. Tomov
								var nm = $t.p.colModel[i].name.replace(/\./g, "\\.");
								if(jQuery("#col_" + nm,"#"+dtbl).attr("checked")) {
									jQuery($t).jqGrid("showCol",$t.p.colModel[i].name);
									jQuery("#col_" + nm,"#"+dtbl).attr("defaultChecked",true); // Added from T. Tomov IE BUG
								} else {
									jQuery($t).jqGrid("hideCol",$t.p.colModel[i].name);
									jQuery("#col_" + nm,"#"+dtbl).attr("defaultChecked",""); // Added from T. Tomov IE BUG
								}
							}
						}
						if(p.ShrinkToFit===true) {
							jQuery($t).jqGrid("setGridWidth",$t.grid.width-0.001,true);
						}
						if(p.closeAfterSubmit) jQuery.jgrid.hideModal("#"+IDs.themodal,{gb:"#gbox_"+gID,jqm:p.jqModal, onClose: p.onClose});
						if (onAfterSubmit) { p.afterSubmitForm(jQuery("#"+dtbl)); }
						return false;
					});
				} else {
					jQuery(":input","#"+dtbl).click(function(e){
						var cn = this.id.substr(4);
						if(cn){
							if(this.checked) {
								jQuery($t).jqGrid("showCol",cn);
							} else {
								jQuery($t).jqGrid("hideCol",cn);
							}
							if(p.ShrinkToFit===true) {
								jQuery($t).jqGrid("setGridWidth",$t.grid.width-0.001,true);
							}
				...
				jQuery("#eData", "#"+dtbl+"_2").click(function(e){
					jQuery.jgrid.hideModal("#"+IDs.themodal,{gb:"#gbox_"+gID,jqm:p.jqModal, onClose: p.onClose});
					return false;
				});
				jQuery("#dData, #eData","#"+dtbl+"_2").hover(
				   function(){jQuery(this).addClass('ui-state-hover');}, 
				   function(){jQuery(this).removeClass('ui-state-hover');}
				);				
				if(onBeforeShow) { p.beforeShowForm(jQuery("#"+dtbl)); }
				jQuery.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, jqM: true, modal:p.modal});
				if(onAfterShow) { p.afterShowForm(jQuery("#"+dtbl)); }
			}
		});
	}
